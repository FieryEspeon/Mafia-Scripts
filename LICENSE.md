...
this.afterChannelJoin = function(src, channel) {
    if (channel == mafiachan) {
        sys.sendHtmlMessage(src, "<strong style=\"color:#3caa67;font-size:13px;\"><timestamp/> ±Murkrow:</strong> <span style=\"font-size:13px;\">Hello, "+html_escape(sys.name(src))+", welcome to Mafia. Use the commands /join a game, and /start [theme] or /vote [theme] to start a game! For more information on how to play, use this guide: <a href=\"http://icekirby.github.io/PO-stuff/beginner/\">http://icekirby.github.io/PO-stuff/beginner/</a>. Use /commands mafia to view a list of other helpful commands. Have fun!</span>", channel);
